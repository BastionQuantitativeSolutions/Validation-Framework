"""
# Author: JG
Validate all 6 pairs - only trade when model signal matches actual TP/SL hit
"""
...
def load_models(model_dir):
    ...
    if lgb_path.exists():
        ...
        lgb_model = lgb.Booster(model_file=str(model_dir / "lightgbm_model.pkl"))
    ...
    if cat_path.exists():
        ...
        cat_model = CatBoostClassifier()
        cat_model.load_model(str(model_dir / "catboost_model.pkl"))
...
print("=" * 70)
print("VALIDATION: All 6 Pairs (Corrected)")
print("Only trade when: (1) model signals direction AND (2) bar hits TP/SL")
print(f"Spread: {SPREAD_PIPS} pips")
print("=" * 70)

results = []
for pair in PAIRS:
    models, scaler, features = load_models(OUTPUT_DIR / f"{pair}_M1")
    if models is None:
        print(f"{pair}: NO MODELS")
        continue
    ...
    lgb_preds = models["lgb"].predict(Xs)
    xgb_preds = models["xgb"].predict(Xs)
    cat_preds = models["cat"].predict(Xs)
...
print(f"{'TOTAL':8} | {total_trades:5} trades | {avg_wr:5.1f}% WR | {total_pnl:+7.1f}R")
print("=" * 70)

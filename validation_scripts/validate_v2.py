"""
# Author: JG
Validate v2 models trained on all bars
"""
...
print("=" * 70)
print("VALIDATION: v2 Models (Trained on ALL bars with class balancing)")
print("Trade when: model predicts Long (>0.5) or Short (<0.5)")
...
for pair in PAIRS:
    ...
    print(f"{pair:8} | {trades:5} trades | {wr:5.1f}% WR | {pnl_r:+7.1f}R | Long: {long_signals}, Short: {short_signals}")

total_trades = sum(r["trades"] for r in results)
total_pnl = sum(r["pnl"] for r in results)
avg_wr = sum(r["wr"] * r["trades"] for r in results) / total_trades if total_trades > 0 else 0
total_long = sum(r["long"] for r in results)
total_short = sum(r["short"] for r in results)
sep = "=" * 70
print(sep)
print(
    "TOTAL     | "
    + str(total_trades).rjust(5)
    + " trades | "
    + f"{avg_wr:5.1f}% WR | "
    + f"{total_pnl:+7.1f}R | Long: {total_long}, Short: {total_short}"
)
print(sep)

    ...
    for tier in ["full", "core", "minimal"]:
        ...
        if xgb_json.exists() and not (tier_dir / "xgb_model.joblib").exists():
            try:
                ...
                # Extract feature names from JSON so align_df uses the booster's
                # expected features, not the stale features.pkl.
                _xgb_feats = None
                try:
                    with open(xgb_json, "r") as _xj:
                        _xgb_meta = json.load(_xj)
                    _xgb_feats = _xgb_meta.get("learner", {}).get("feature_names")
                except Exception:
                    pass
                # Use JSON feature names if available; otherwise fall back to tier_feats
                _alignment_feats = _xgb_feats if _xgb_feats else tier_feats
                # Wrap in a sklearn-compatible shim so predict_proba works
                class _XGBBoosterShim:
                    def __init__(self, booster, feat_names):
                        self._b = booster
                        self.feature_names_in_ = np.array(feat_names) if feat_names else None
                ...
                families["xgb"].append((_XGBBoosterShim(m, _alignment_feats), _alignment_feats))
                log.debug(f"[MODEL] Loaded {pair}_{tf} {tier}/xgb.json ({len(_alignment_feats) if _alignment_feats else 'all'} feats)")
            except Exception as e:
...
def rolling_trade_simulation(
    ...
    for i in range(test_start, test_end):
        ...
        # SL/TP using FIXED PERCENTAGES matching training labels (TP_PCT=0.003, SL_PCT=0.0015)
        # Training labels were created with: tp = entry*(1+TP_PCT), sl = entry*(1-SL_PCT)
        # Using ATR-based exits misaligns validation from what models were trained to predict.
        entry_price = float(df.iloc[i]["close"])
        if direction == 1:
            sl = entry_price * (1 - TRAIN_SL_PCT)
            tp = entry_price * (1 + TRAIN_TP_PCT)
        else:
            sl = entry_price * (1 + TRAIN_SL_PCT)
            tp = entry_price * (1 - TRAIN_TP_PCT)
...
def main():
    ...
    out_path = OUTPUT_DIR / f"live_equivalent_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

...
Properly handles the model structure from load_tiered_models.
"""
        ...
        if not LIVE_MODULES_AVAILABLE:
            ...
            try:
                ...
                if models_tuple and len(models_tuple) == 3:
                    ...
                    for models_dict, tier_name in [(full_models, "full"), (core_models, "core")]:
                        if models_dict and "full" in models_dict:
                            tier_data = models_dict["full"]
        ...
        for model in models:
            try:
                if hasattr(model, "predict_proba"):
                    proba = model.predict_proba(last_features_numeric)
                    if len(proba) > 0 and len(proba[0]) == 2:
                        predictions.append(proba[0][1])
                elif hasattr(model, "predict"):
                    pred = model.predict(last_features_numeric)
                    if len(pred) > 0:
...
if __name__ == "__main__":
    ...
    logger.info("Running fixed ML backtest on first 5 pairs...")

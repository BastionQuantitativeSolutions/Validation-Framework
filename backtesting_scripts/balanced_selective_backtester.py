#!/usr/bin/env python3
"""
Balanced Selective Backtester
==============================

Uses balanced filters to achieve 70% win rate while still generating enough signals.
"""
...
MAX_DAILY_DD = 0.03
MIN_PROFIT = 0
SIGNAL_INTERVAL = 2  # Check every 2 bars (30 min on M15) for more opportunities

# Balanced filters - less strict than ultra but more selective than default
MIN_CONFIDENCE = 0.55  # Only take signals with 55%+ confidence
MIN_ML_PROB_DIFF = 0.08  # ML probability must be at least 0.08 away from 0.5
...
def generate_signal(df, idx, pair):
    ...
    try:
        ...
        # Check ML probability difference
        ml_prob_diff = abs(ml_prob - 0.5)
        if ml_prob_diff < MIN_ML_PROB_DIFF:
            return None
...
def run_backtest(all_data, balance=10000, risk_per_trade=0.75, signal_interval=SIGNAL_INTERVAL, max_pairs=5):
    total_trades = 0
    winning_trades = 0
    total_pnl = 0.0
    max_drawdown = 0.0
...
if __name__ == "__main__":
    ...
    logger.info("Running balanced selective ML backtest on first 5 pairs...")
    logger.info(f"Filters: MIN_CONFIDENCE={MIN_CONFIDENCE}, MIN_ML_PROB_DIFF={MIN_ML_PROB_DIFF}")
    results = run_backtest(all_data, balance=10000, risk_per_trade=0.75, signal_interval=SIGNAL_INTERVAL, max_pairs=5)

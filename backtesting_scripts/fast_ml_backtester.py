...
Samples every Nth bar for signals to achieve reasonable runtime.
"""
...
MAX_DAILY_DD = 0.03
MIN_PROFIT = 0
SIGNAL_INTERVAL = 8  # Check for signals every 8 bars (2 hours on M15)
                    ...
                    for models_dict, tier_name in [(full_models, "full"), (core_models, "core"), (minimal_models, "minimal")]:
                        if models_dict:
                            for model_name, model in models_dict.items():
                                all_models[f"{tier_name}_{model_name}"] = model

                    MODELS_CACHE[cache_key] = all_models
                    logger.info(f"Cached {len(all_models)} models for {pair}_{tf}")
                else:
                    MODELS_CACHE[cache_key] = None
...
def generate_signal(df, idx, pair):
    """Generate signal using cached ML models."""
    if idx < 200:
        return None

    models = get_models_cached(pair, "M15")
...
def run_backtest(all_data, balance=10000, risk_per_trade=0.75, signal_interval=SIGNAL_INTERVAL):
    total_trades = 0
    winning_trades = 0
    total_pnl = 0.0
    max_drawdown = 0.0
    ...
    for symbol, df in all_data.items():
        ...
        logger.info(f"Processing {symbol} ({len(df)} bars, checking every {signal_interval} bars)...")

        # Only check every Nth bar for signals
        for i in range(200, len(df), signal_interval):
...
if __name__ == "__main__":
    ...
    logger.info(f"Running fast backtest (checking every {SIGNAL_INTERVAL} bars)...")
    results = run_backtest(all_data, balance=10000, risk_per_trade=0.75, signal_interval=SIGNAL_INTERVAL)
    ...
    if targets_met:
        logger.info("\n*** TARGETS MET! ***")
        output_file = Path("./sample_project/CORE_MODULES/config/backtest_results.json")
        with open(output_file, "w") as f:
            json.dump(
                {"timestamp": datetime.now().isoformat(), "results": results, "targets_met": True, "signal_interval": SIGNAL_INTERVAL}, f, indent=2
            )

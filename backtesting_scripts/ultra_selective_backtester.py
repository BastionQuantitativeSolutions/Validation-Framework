#!/usr/bin/env python3
"""
Ultra Selective Backtester
==========================

Only takes the highest confidence ML signals to achieve 70%+ win rate.
"""
...
MIN_CONFIDENCE = 0.65  # Only take signals with 65%+ confidence
MIN_ML_PROB_DIFF = 0.15  # ML probability must be at least 0.15 away from 0.5
...
def generate_signal(df, idx, pair):
    ...
    try:
        ...
        # Ultra selective: only take extreme probabilities
        ml_prob_diff = abs(ml_prob - 0.5)
        if ml_prob_diff < MIN_ML_PROB_DIFF:
            return None

        if fused >= BASE_BUY_THRESHOLD + 0.1:  # Higher than base threshold
            direction = 1
        elif fused <= BASE_SELL_THRESHOLD - 0.1:  # Lower than base threshold
            direction = -1
        else:
            return None
        ...
        # Ultra selective confidence filter
...
if __name__ == "__main__":
    ...
    logger.info("Running ultra selective ML backtest on first 3 pairs...")
